/*
 * The Farmer's Pickup - a pickup for Vanilla Wheels.
 ...
 */
package com.chunkworks.farmpickup.gametest;
...
/**
 * The Farmer's Pickup on a headless server: its profile is registered with two
 * seats, one driver, a tailgate and a chest of six rows; the truck reaches speed on the
 ...
 * the gauge reads; the chassis crafts from eight iron blocks round a hay bale and names the
 * truck.
 */
@GameTestHolder("farmpickup")
@PrefixGameTestTemplate(false)
public final class PickupGameTests {
    private static final int LENGTH = 48;
    private static final int WIDTH = 15;
    private static final int FLOOR = 4;
    private static final ResourceLocation TRUCK = ResourceLocation.fromNamespaceAndPath("farmpickup", "pickup");
    ...
    private static Vehicle truck(GameTestHelper helper, double x, double z) {
        ...
        helper.assertTrue(v != null, "the Farmer's Pickup is registered");
    ...
    public void theProfileIsRegisteredWithFourSeatsADriverAChestAndAHitch(GameTestHelper helper) {
        ...
        helper.assertTrue(v.getName().getString().equals("Farmer's Pickup"), "named: " + v.getName().getString());
        helper.succeed();
    }
    ...
    @GameTest(template = "arena", timeoutTicks = 60)
    public void theChassisCraftsFromEightIronBlocksRoundAHayBaleAndNamesTheTruck(GameTestHelper helper) {
        NonNullList<ItemStack> grid = NonNullList.withSize(9, ItemStack.EMPTY);
        for (int i = 0; i < 9; i++) {
            grid.set(i, new ItemStack(i == 4 ? Items.HAY_BLOCK : Items.IRON_BLOCK));
        }
        ...
        helper.assertTrue(recipe.isPresent(), "eight iron blocks round a hay bale craft something");
        helper.assertValueEqual(recipe.get().id(), ResourceLocation.fromNamespaceAndPath("farmpickup", "pickup_chassis"), "the Farmer's Pickup's chassis recipe");
        ...
        helper.assertValueEqual(VanillaWheels.vehicleOf(result).orElse(null), TRUCK, "for the Farmer's Pickup");
        helper.assertTrue(result.getHoverName().getString().contains("Farmer's Pickup"), "named for it: " + result.getHoverName().getString());
        helper.succeed();
    }
